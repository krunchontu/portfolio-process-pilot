 ...
 *
 * Purpose: Edit existing workflows with complete step management
 *
 * Features:
...
const EditWorkflowModal = ({ isOpen, onClose, workflow }) => {
  ...
  // Pre-populate form when workflow changes
  useEffect(() => {
    if (workflow) {
      reset({
        name: workflow.name || '',
        flowId: workflow.flowId || workflow.flow_id || '',
        ...
        steps: Array.isArray(workflow.steps)
          ? workflow.steps.map((step, index) => ({
              ...
              actions: Array.isArray(step.actions) ? step.actions : ['approve', 'reject'],
  ...
  const updateWorkflowMutation = useMutation(
    (data) => workflowsAPI.update(workflow.id, data),
    {
      onSuccess: () => {
      ...
      onError: (error) => {
        const message = error.response?.data?.error || 'Failed to update workflow'
        toast.error(message)
      }
    }
  )
  ...
  const handleClose = () => {
    if (!updateWorkflowMutation.isLoading) {
      reset()
      onClose()
    }
  }
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        {...register('flowId')}
                        disabled
                        title="Flow ID cannot be changed"
                      />
                      <p className="mt-1 text-xs text-secondary-500">
                        Flow ID cannot be changed after creation
                      </p>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <label htmlFor="isActive" className="ml-2 text-sm text-secondary-700">
                        Active (workflow can be used for new requests)
                      </label>
                    </div>
                  </div>
